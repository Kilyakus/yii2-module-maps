            ...
            <thead>
                <tr>
                    <?php if(IS_ROOT) : ?>
                        <!-- <th width="50">#</th> -->
                    <?php endif; ?>
                    <th width="60"><?= Yii::t('easyii', 'Code') ?></th>
                    <th><?= Yii::t('easyii', 'Title') ?></th>
                    <th width="100" class="text-center"><?= Yii::t('easyii', 'Regions') ?></th>
            ...
            <tbody>
                <?php foreach($data->models as $item) : ?>
                    <tr data-id="<?= $item->primaryKey ?>">
                        ...
                        <td class="text-muted">
                            <div class="row">
                                <div class="col-sm-4 col-xs-4 hidden-lg hidden-md"><strong><?= Yii::t('easyii', 'Code') ?></strong></div>
                                <div class="col-lg-12 col-md-12 col-sm-8 col-xs-8"><?= $item->code ?></div>
                            </div>
                        </td>
                        <td>
                            <div class="row">
                        ...
                        <td>
                            <div class="row">
                                <div class="col-sm-4 col-xs-4 hidden-lg hidden-md"><strong><?= Yii::t('easyii', 'Regions') ?></strong></div>
                                <div class="col-lg-12 col-md-12 col-sm-8 col-xs-8">
                                    <a class="btn btn-sm btn-default btn-block" <?= !count($item->regions) ? 'disabled="true"' : 'href="'.Url::toRoute(['/admin/'.$module.'/a/regions/', 'id' => $item->primaryKey]).'"'; ?>><?= count($item->regions) ?></a>
                                </div>
                            </div>
                        </td>
                        <td>
                            <div class="row">
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-8 col-xs-8">
                                    <a class="btn btn-sm btn-default btn-block" <?= !count($item->cities) ? 'disabled="true"' : 'href="'.Url::toRoute(['/admin/'.$module.'/a/cities/', 'id' => $item->primaryKey]).'"'; ?>><?= count($item->cities) ?></a>
                                </div>
                            </div>
                        </td>
                        <td>
